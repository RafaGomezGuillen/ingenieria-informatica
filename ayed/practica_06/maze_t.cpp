...
bool maze_t::solve_(const int i, const int j) {
  ...
  if (is_ok_(i - 1, j)) {
    if (solve_(i - 1, j)) {
      ...
      lista.insert_head(new dll_node_t<char>('N'));
      return true;
    }
  }
  // Este
  if (is_ok_(i, j + 1)) {
    if (solve_(i, j + 1)) {
      ...
      lista.insert_head(new dll_node_t<char>('E'));
      return true;
    }
  }
  // Sur
  if (is_ok_(i + 1, j)) {
    if (solve_(i + 1, j)) {
      matrix_(i + 1, j) = PATH_ID;
      lista.insert_head(new dll_node_t<char>('S'));
      return true;
    }
  }
  // Oeste
  if (is_ok_(i, j - 1)) {
    if (solve_(i, j - 1)) {
      matrix_(i, j - 1) = PATH_ID;
      lista.insert_head(new dll_node_t<char>('O'));
      return true;
    }
  }
  ...
  if (is_ok_(i + 1, j + 1)) {
    if (solve_(i + 1, j + 1)) {
      matrix_(i + 1, j + 1) = PATH_ID;
      lista.insert_head(new dll_node_t<char>('R'));
      return true;
    }
  }
  // Sur oeste
  if (is_ok_(i + 1, j - 1)) {
    if (solve_(i + 1, j - 1)) {
      matrix_(i + 1, j - 1) = PATH_ID;
      lista.insert_head(new dll_node_t<char>('H'));
      return true;
    }
  }
  // Nor este
  if (is_ok_(i - 1, j + 1)) {
    if (solve_(i - 1, j + 1)) {
      matrix_(i - 1, j + 1) = PATH_ID;
      lista.insert_head(new dll_node_t<char>('L'));
      return true;
    }
  }
  // Nor oeste
  if (is_ok_(i - 1, j - 1)) {
    if (solve_(i - 1, j - 1)) {
      matrix_(i - 1, j - 1) = PATH_ID;
      lista.insert_head(new dll_node_t<char>('T'));
      return true;
    }
  }

  contador_backtrack++;  // contador del numero de retrocesos
...
void maze_t::maze_path(void) {
  if (lista.empty()) {
    ...
    cout << "Existe un camino con las siguientes coordenadas, partiendo desde "
            "A y ha retrocedido (backtrack): "
         << contador_backtrack << endl;
    if (aux) {
      cout << "A: ";
      while (aux) {
        cout << aux->get_data() << "-->";
